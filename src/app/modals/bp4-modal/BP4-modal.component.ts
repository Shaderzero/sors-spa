...
@Component({
  selector: 'app-bp4-modal',
  templateUrl: './BP4-modal.component.html',
  styleUrls: ['./BP4-modal.component.css']
})
export class BP4ModalComponent implements OnInit {
  bpForm: FormGroup;
  buttonName: string;
  editMode: boolean;
  bps: BusinessProcess[];
  ...
  ngOnInit() {
    this.fillBps1();
    if (this.editMode) {
      this.patchBps2(this.bp.parent.parent.parentId);
      this.patchBps3(this.bp.parent.parentId);
    }
    this.createBpForm();
  }
  ...
  patchCode(bp3Id) {
    for (let i = 0; i < this.bps3.length; i++) {
      if (this.bps3[i].id === bp3Id) {
        this.bpForm.patchValue({code: this.bps3[i].code});
        break;
      }
    }
  }
  ...
  createBpForm() {
    this.bpForm = this.fb.group({
      ...
      bp1Id: [this.bp.parent.parent.parentId, Validators.required],
      bp2Id: [this.bp.parent.parentId, Validators.required],
