...
@Injectable()
export class RiskStatusesResolver implements Resolve<RiskStatus[]> {

    constructor(private refService: ReferenceService, private route: Router, private alertify: AlertifyService) {}

    resolve(route: ActivatedRouteSnapshot): Observable<RiskStatus[]> {
        return this.refService.getRiskStatuses().pipe(
            catchError(error => {
