...
@Injectable()
export class BusinessProcessesResolver implements Resolve<BusinessProcess[]> {

    constructor(private refService: ReferenceService, private route: Router, private alertify: AlertifyService) {}

    resolve(route: ActivatedRouteSnapshot): Observable<BusinessProcess[]> {
        return this.refService.getBusinessProcesses().pipe(
            catchError(error => {
